...
return ncompile(tail(gtokens(expr)), str("!(", gstr(expr), ")")); })() : ((hd==="lam") ? (function(){var args = compileLambdaArgs(tail(tl));
...
var compileLambdaArgs = function(tokens){ return (empty(tokens) ? err("error: unexpected end") : (function(){var hd = head(tokens);
var tl = tail(tokens);
return ((hd===")") ? nargs(tl, []) : (function(){var rest = compileLambdaArgs(tl);
