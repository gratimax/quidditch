...
(def nextOfComment (lam (res stri)
  (if (empty stri)
...
(def nextOfVal (lam (acc res stri)
  (if (empty stri)
...
(def nextOfString (lam (acc escape res stri)
  (if (empty stri)
...
(def nargs (lam (tokens args)
  (list tokens args)))

; get the args out of the struct
(def gargs (lam (struct)
  (at struct 1)))

(def compileArgs)
(def compileLambdaArgs)
(def compileDo)

(def compileExpr (lam (tokens)
  (if (empty tokens)
          ...
          (if (== hd "def")
            ...
            (if (has infix hd)
              ...
              (if (== hd "!")
                ...
                (if (== hd "lam")
                  (do
                    (def args (compileLambdaArgs (tail tl)))
...
(def compileLambdaArgs (lam (tokens)
  (if (empty tokens)
    (err "error: unexpected end")
    (do
      (def hd (head tokens))
      ...
      (if (== hd ")")
        (nargs tl nil)
        (do
          (def rest (compileLambdaArgs tl))
          (nargs (gtokens rest) (cons hd (gargs rest)))))))))

(def compileDo (lam (stmts tokens)
  (if (empty tokens)
    (err "error: unexpected end")
    (do
      (def hd (head tokens))
