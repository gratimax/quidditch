...
var set = function(obj, key, value){ return (function(){
obj[key]=value;
return obj;})(); };
...
var toObjStr = function(obj){ return Object.prototype.toString.call(obj); };
...
var isArray = function(obj){ return (toObjStr(obj)==="[object Array]"); };
var isString = function(obj){ return (toObjStr(obj)==="[object String]"); };
var parens = list("(", ")");
var whitespace = list(" ", "\n");
...
var macros = object();
set(macros, "!", function(tree){ return ((len(tree)===1) ? str("!(", compileExpr(head(tree)), ")") : err("! takes one arg")); });
var infix = list("+", "-", "/", "*", "&&", "||", "<", ">", "<=", ">=");
foreach(infix, function(infix){ return set(macros, infix, function(tree){ return ((len(tree)===2) ? str("(", compileExpr(head(tree)), infix, compileExpr(at(tree, 1)), ")") : err(str(infix, " takes two args"))); }); });
set(macros, "==", function(tree){ return ((len(tree)===2) ? str("(", compileExpr(head(tree)), "===", compileExpr(at(tree, 1)), ")") : err("== takes two args")); });
set(macros, "!=", function(tree){ return ((len(tree)===2) ? str("(", compileExpr(head(tree)), "!==", compileExpr(at(tree, 1)), ")") : err("!= takes two args")); });
set(macros, "mod", function(tree){ return ((len(tree)===2) ? str("(", compileExpr(head(tree)), "%", compileExpr(at(tree, 1)), ")") : err("!= takes two args")); });
set(macros, "def", function(tree){ return ((len(tree)===2) ? str("var ", unquote(head(tree)), " = ", compileExpr(at(tree, 1))) : err("def takes two args")); });
set(macros, "if", function(tree){ return ((len(tree)===3) ? str("(", compileExpr(head(tree)), " ? ", compileExpr(at(tree, 1)), " : ", compileExpr(at(tree, 2)), ")") : err("if takes three args")); });
set(macros, "do", function(tree){ return ((len(tree)===0) ? err("do takes at least one arg") : (function(){
...
set(macros, "fn", function(tree){ return ((len(tree)===2) ? (function(){
...
set(macros, "defn", function(tree){ return ((len(tree)===3) ? (function(){
var name = head(tree);
var args = at(tree, 1);
var body = at(tree, 2);
var anon = list(quote("fn"), args, body);
return compileExpr(list(quote("def"), name, anon));})() : err("defn takes three args")); });
set(macros, "let", function(tree){ return ((len(tree)===0) ? err("let takes at least one arg") : (function(){
var expr = last(tree);
...
set(macros, "cond", function(tree){ return ((len(tree)===0) ? err("cond takes at least two args") : (function(){
var lt = last(tree);
